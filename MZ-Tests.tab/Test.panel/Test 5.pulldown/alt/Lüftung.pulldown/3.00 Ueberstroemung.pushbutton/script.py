        ...
        for Item in Familie:
            ...
            if Item.Space[phase]:
                ...
                if System_Typ == '31_Überstromluft':
                    ...
                    logger.error(System_Typ, 'Id: ', int(Item.Id.ToString()))
            ...
            for item in input_list:
                ...
                if System_Name == item[0] and System_Typ == '31_Überstromluft':
                    ...
                    try:
                        ...
                        Error = Error + '\n' + System_Name + ': Kein Ein/Ausgangsraum gefunden. ElementID: ' + Item.Id.ToString()
                        logger.error(
                            System_Name + ': Kein Ein-/Ausgangsraum gefunden.  ElementID: ' + Item.Id.ToString())
        if Error:
...
def Raum_Daten(Eingang, Ausgang, Ein_Raum, Aus_Raum, Raum):
    ...
    for item in Raumdaten:
        ...
        if len(item[1]) > 0:
            text_ein = 'Aus: [m3/h]--'
            pruefen_Nr_aus = item[1][0]
            vol = 0
            for n, i in enumerate(item[1]):
                if pruefen_Nr_aus == i[0]:
                    vol += i[1]
                else:
                    text_ein = text_ein + pruefen_Nr_aus + ': ' + str(int(vol)) + ', '
                    pruefen_Nr = i[0]
                    vol = i[1]
                if n == len(i) - 1:
                    text_ein = text_ein + pruefen_Nr_aus + ': ' + str(int(vol)) + ', '
        ...
        if len(item[2]) > 0:
            text_aus = 'In: [m3/h]--'
            pruefen_Nr_in = item[2][0]
            vol_in = 0
            for n, i in enumerate(item[2]):
                if pruefen_Nr_in == i[0]:
                    vol_in += i[1]
                else:
                    text_aus = text_aus + pruefen_Nr_in + ': ' + str(int(vol_in)) + ', '
                    pruefen_Nr = i[0]
                    vol_in = i[1]
                if n == len(i) - 1:
                    text_aus = text_aus + pruefen_Nr_in + ': ' + str(int(vol_in)) + ', '
...
Eingangsdaten, Ausgangsdaten, Raumdaten, Raum_Text = Raum_Daten(Eingang_liste, Ausgang_liste, Ein_Raum_liste,
                                                                Aus_Raum_liste, Raum_liste)
