...
__doc__ = """Schacht von Lüftung Übernehmen und ULK Kälte-Leistung von MEP-Raum bzw. Wassermenge von Schacht berechnen"""
...
for i in range(len(Schacht)):
    ULK_VL_Liste.append(0)
    ULK_RL_Liste.append(0)
    ...
    for space in spaces_collector:
        ...
        for ii in range(len(Schacht)):
            if ULK_V_S == Schacht[ii]:
                ULK_VL_Liste[ii] = Wassermenge + ULK_VL_Liste[ii]
            if ULK_R_S == Schacht[ii]:
                ULK_RL_Liste[ii] = Wassermenge + ULK_RL_Liste[ii]

table_data_Schacht = []
for Nr in range(len(Schacht)):
    table_data_Schacht.append([Schacht[Nr], ULK_VL_Liste[Nr], ULK_RL_Liste[Nr],])
...
output.print_table(
    ...
    title="Schächte",
    columns=[ 'Schacht','ULK-VL_Win_Menge','ULK-RL_Win_Menge',  ]
)
...
#                        Schacht_V.SetValueString(str(ULK_VL_Liste[num]))
#                        Schacht_R.SetValueString(str(ULK_RL_Liste[num]))
                        SchachtV.SetValueString(str(ULK_VL_Liste[num]))
                        SchachtR.SetValueString(str(ULK_RL_Liste[num]))
