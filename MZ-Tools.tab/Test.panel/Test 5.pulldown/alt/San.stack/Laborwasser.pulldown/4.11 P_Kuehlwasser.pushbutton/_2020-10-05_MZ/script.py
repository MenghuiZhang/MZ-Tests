...
__title__ = "P_KüW"
__doc__ = """Schacht von Lüftung Übernehmen und PKW-Leistung von MEP-Raum bzw. Wassermenge von Schacht berechnen"""
...
for i in range(len(Schacht)):
    PKW_VL_Liste.append(0)
    PKW_RL_Liste.append(0)
    ...
    for space in spaces_collector:
        ...
        for ii in range(len(Schacht)):
            if PKW_V_S == Schacht[ii]:
                PKW_VL_Liste[ii] = Wassermenge + PKW_VL_Liste[ii]
            if PKW_R_S == Schacht[ii]:
                PKW_RL_Liste[ii] = Wassermenge + PKW_RL_Liste[ii]

table_data_Schacht = []
for Nr in range(len(Schacht)):
    table_data_Schacht.append([Schacht[Nr], PKW_VL_Liste[Nr], PKW_RL_Liste[Nr],])

output.print_table(
    table_data=table_data,
    title="Mep Räume",
    columns=['Nummer', 'Name',  'Schacht_VL', 'Schacht_RL', 'Wassermenge',
             'Temp_VL', 'Temp_RL', 'PKW_Leistung', ]
)

output.print_table(
    table_data=table_data_Schacht,
    title="Schächte",
    columns=[ 'Schacht','PKW-VL_Win_Menge','PKW-RL_Win_Menge',  ]
)
            ...
            if Ins_Schacht:
                for num in range(len(Schacht)):
                    if Ins_Schacht_Name == Schacht[num]:
                        ...
                        SchachtV.SetValueString(str(PKW_VL_Liste[num]))
                        SchachtR.SetValueString(str(PKW_RL_Liste[num]))
