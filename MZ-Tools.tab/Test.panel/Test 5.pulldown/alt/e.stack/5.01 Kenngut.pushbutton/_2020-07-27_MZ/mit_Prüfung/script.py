...
if not ducts:
    logger.error("Keine Luftkanalzubehör in aktueller Projekt gefunden")
    script.exit()
...
class LUFTKanalzubehoer:
    def __init__(self, element_id):
        ...
        KG_Liste = [self.KG_GW, self.KG_BN, self.KG_EB, self.KG_A1, self.KG_A2, self.KG_DP, self.KG_DPZ]

        # Prüfung
        if not all(KG_Liste):
            logger.error("Es fehlt Information in System {}, Kennzeichen {}".format(self.name, self.KZ) )
            script.exit()
