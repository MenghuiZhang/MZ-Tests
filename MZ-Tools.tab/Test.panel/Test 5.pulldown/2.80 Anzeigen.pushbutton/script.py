...
class Familienauswahl(WPFWindow):
    ...
    def read_config(self):
        try:
            for el in config.HFamilie:
                if len(config.HFamilie) == 0:
                    break
                self.ListeH.Add(H_Bauteile(el[0],el[1]))
    ...
    def write_config(self):
        try:
            liste = []
            for el in self.ListeH:
                liste.append([el.Famname,el.HLeistung])
            config.HFamilie = liste
        except:
            pass
        script.save_config()
