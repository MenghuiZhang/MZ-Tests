...
def Pickelem():
    try:
        re = revit.uidoc.Selection.PickObject(DB.Selection.ObjectType.Element)
...
def AlleElementInAnsicht():
    ...
    try:
        ...
        coll = DB.FilteredElementCollector(revit.doc,revit.active_view.Id).OfCategory(Category).WhereElementIsNotElementType()

        for elem in coll:
            fa_name = elem.Parameter[DB.BuiltInParameter.ELEM_FAMILY_PARAM].AsValueString()
            ty_name = elem.Parameter[DB.BuiltInParameter.ELEM_TYPE_PARAM].AsValueString()
...
def AlleElementInDocument():
    ...
    try:
        ...
        familie = element.Parameter[DB.BuiltInParameter.ELEM_FAMILY_PARAM].AsValueString()
        typ = element.Parameter[DB.BuiltInParameter.ELEM_TYPE_PARAM].AsValueString()
        coll = DB.FilteredElementCollector(revit.doc).OfCategory(Category).WhereElementIsNotElementType()
