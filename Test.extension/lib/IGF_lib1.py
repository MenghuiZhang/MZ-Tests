...
def ViewFilter_Erstellen(Systemtyp):
    doc = Systemtyp.Document
    Filters_coll = DB.FilteredElementCollector(doc).OfClass(DB.FilterElement).ToElements()
    FILTERS = {i.Name :i for i in Filters_coll}

    Cate = List[DB.ElementId]()
    filterRules = List[DB.FilterRule]()
    systemtypename = Systemtyp.GetType().Name
    if  systemtypename == 'PipingSystemType':
        Cate.Add(doc.Settings.Categories.get_Item(DB.BuiltInCategory.OST_PipeInsulations).Id)
        Cate.Add(doc.Settings.Categories.get_Item(DB.BuiltInCategory.OST_PipeAccessory).Id)
        Cate.Add(doc.Settings.Categories.get_Item(DB.BuiltInCategory.OST_FlexPipeCurves).Id)
        Cate.Add(doc.Settings.Categories.get_Item(DB.BuiltInCategory.OST_PipeFitting).Id)
        Cate.Add(doc.Settings.Categories.get_Item(DB.BuiltInCategory.OST_PipeCurves).Id)
    else:
        Cate.Add(doc.Settings.Categories.get_Item(DB.BuiltInCategory.OST_DuctInsulations).Id)
        Cate.Add(doc.Settings.Categories.get_Item(DB.BuiltInCategory.OST_FlexDuctCurves).Id)
        Cate.Add(doc.Settings.Categories.get_Item(DB.BuiltInCategory.OST_DuctAccessory).Id)
        Cate.Add(doc.Settings.Categories.get_Item(DB.BuiltInCategory.OST_DuctFitting).Id)
        Cate.Add(doc.Settings.Categories.get_Item(DB.BuiltInCategory.OST_DuctCurves).Id)
    name = Systemtyp.get_Parameter(DB.BuiltInParameter.SYMBOL_NAME_PARAM).AsString()
    if name in FILTERS.keys():
        _filter = FILTERS[name]
        _filter.SetCategories(Cate)
        _filter.ClearRules()
    
    else:
        _filter = DB.ParameterFilterElement.Create(doc, name, Cate)
    if systemtypename == 'PipingSystemType':
    ...
    doc.Regenerate()
    filterRules.Add(DB.ParameterFilterRuleFactory.CreateEqualsRule(paramelemid, Systemtyp.Id))
    _filter.SetElementFilter( DB.ElementParameterFilter(filterRules))
    del Cate
    doc.Dispose()
    del doc
    del filterRules
...
def ViewFilterInViewHinzufügen(Systemtyp,Filter,View):
    filter_liste = {Systemtyp.Document.GetElement(filterid).Name: Systemtyp.Document.GetElement(filterid) for filterid in View.GetFilters()}
    if Filter.Name in filter_liste.keys():
        pass
    else:
        View.AddFilter(Filter.Id)
    overrideGraphicSettings = DB.OverrideGraphicSettings()
    if Systemtyp.LinePatternId:
        overrideGraphicSettings.SetProjectionLinePatternId(Systemtyp.LinePatternId)
    if Systemtyp.LineColor.IsValid:
        color = Color.FromArgb(Systemtyp.LineColor.Red,Systemtyp.LineColor.Green,Systemtyp.LineColor.Blue)
        h = color.GetHue() * 240 / 360.0
        s = color.GetSaturation() * 240
        l = color.GetBrightness() * 240
        if l > 40:
            r,g,b = hsl2rgb(h,s,l-40)
        else:
            r,g,b = hsl2rgb(h,s,l/2.0)
        overrideGraphicSettings.SetProjectionLineColor(DB.Color(Byte(r),Byte(g),Byte(b)))
        overrideGraphicSettings.SetSurfaceForegroundPatternColor(Systemtyp.LineColor)
        overrideGraphicSettings.SetSurfaceForegroundPatternId(DB.FillPatternElement.GetFillPatternElementByName(Systemtyp.Document,DB.FillPatternTarget.Drafting,'<Flächenfüllung>').Id)
    View.SetFilterOverrides(Filter.Id,overrideGraphicSettings)
    overrideGraphicSettings.Dispose()
    del filter_liste
    del overrideGraphicSettings