    ...
    def config_ermitterl(self):
        ...
        if not os.path.exists(self._cfg_file_path_cental):
            ...
            if not os.path.exists(self.config_rvt):
                try: os.makedirs(self.config_rvt)
                except:
                    try:
                        self.lokal = True
                        if not os.path.exists(self._cfg_file_path_local):
                            self._cfg_file_path = self._cfg_file_path_local
                            if not os.path.exists(self.configfile):
                                try:   os.makedirs(self.configfile)
                                except:
                                    self.isconfig = False
                                    return
                            else:
                                try:
                                    with open(self._cfg_file_path_local,'w') as file:     pass
                                except:
                                    self.isconfig = False
                                    return
                        else:     return
            ...
            try:
                with open(self._cfg_file_path_cental,'w') as file:pass
            except:
                self.isconfig = False
                return
