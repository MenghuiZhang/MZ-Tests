...
class ProjektConfig(object):
    ...
    def __init__(self,nummername = '',lokal = False):
        ...
        if self.nummername.strip() == '-':
            self.nummername == 'Musternummer - Mustername'
    ...
    def get_value(self,option):
        if self.config.has_option(self.section, option):
            try:
                ...
                try:
                    try: return json.loads(value)  #pylint: disable=W0123
                    except Exception:
                        # try fix legacy formats
                        # cleanup python style true, false values
                        if value == "True":    value = json.dumps(True)
                        elif value == "False": value = json.dumps(False)
    ...
    def get_config(self,section = 'Default'):
        if self.lokal:self.section = self.nummername+section
        else:self.section = section
        if self.config.has_section(self.section):return
        else:self.config.add_section(self.section)
