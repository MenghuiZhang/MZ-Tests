...
config = ProjektConfig.get_config_static(number + ' - ' + name,'Wirkungsort')
...
class FamilieEinstellen(forms.WPFWindow):
    ...
    def read_config(self):
        try:
            for item in self.Liste_EV:
                _Endverbraucher = config.get_value('Endverbraucher')
                if item.familyname in _Endverbraucher.keys():
                    configdatei = _Endverbraucher[item.familyname]
        ...
        try:
            for item in self.Liste_RK:
                _Regelkomponent = config.get_value('Regelkomponent')
                if item.familyname in _Regelkomponent.keys():
                    configdatei = _Regelkomponent[item.familyname]
    ...
    def write_config(self):
        ...
        config.set_value('Endverbraucher', _dict_EV)
        config.set_value('Regelkomponent', _dict_RK)
        config.save_config()
