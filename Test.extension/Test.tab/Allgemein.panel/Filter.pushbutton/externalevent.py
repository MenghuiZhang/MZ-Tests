...
class ExternalEventListe(IExternalEventHandler):
    ...
    def Reset(self,uiapp):
        ...
        for elid in elemids:
            try:
                ...
                try:catname = elem.get_Parameter(DB.BuiltInParameter.ELEM_CATEGORY_PARAM).AsValueString()
                except:catname = 'Sonstige'
    ...
    def Reset1(self,uiapp):
        ...
        for elid in elemids:
            try:
                elem = doc.GetElement(elid)
                if elem.GetType().Name != 'IndependentTag' and elem.GetType().BaseType.Name != 'SpatialElementTag':
                    continue
                ...
                if familyname not in Dict1.keys():
                    Dict1[familyname] = {}
                    Dict[familyname] = {}
    
                if typname not in Dict1[familyname].keys():
                    Dict1[familyname][typname] = {}
                if familienname not in Dict1[familyname][typname].keys():
                    Dict1[familyname][typname][familienname] = {}
                if typenname not in Dict1[familyname][typname][familienname].keys():
                    Dict1[familyname][typname][familienname][typenname] = []
                Dict1[familyname][typname][familienname][typenname].append(elid)

                if familienname not in Dict[familyname].keys():
                    Dict[familyname][familienname] = {}
                if typenname not in Dict[familyname][familienname].keys():
                    Dict[familyname][familienname][typenname] = []
                Dict[familyname][familienname][typenname].append(elid)
            except:pass
        for cate in sorted(Dict.keys()):
            Categoryinstance = Category(cate,Dict[cate],Dict1[cate])
            ItemsSource_Cate.Add(Categoryinstance)
        self.GUI.LV_B_Familien.ItemsSource = ItemsSource_Cate
        self.GUI.temp_coll1.Clear()
        self.GUI.temp_coll_Systemtyp1.Clear()
    ...
    def PostSelect1(self,uiapp):
        self.name = 'ausgewähle Typen in Revit markieren'
        uidoc = uiapp.ActiveUIDocument
        Liste = []
        for el in self.GUI.DG_Familie_B.Items:
            if el.checked:
                Liste.extend(el.elems)
        uidoc.Selection.SetElementIds(List[DB.ElementId](Liste))
        uidoc.Dispose()
