    ...
    try:
        ...
        for i in alltype:
            ...
            if not Type in ['Invalid','FamilyType']:
                type1 = DB.LabelUtils.GetLabelFor(i)
              
                dis = DeiziplinErmitteln(type1,Type)
                allParaType.append([dis,type1,i])
    except:
        alltype = System.Enum.GetValues(DB.ParameterType.Invalid.GetType())
        for i in alltype:
            Type = i.ToString()
            ...
            if not Type in ['Invalid','FamilyType']:
                try:
                    ...
                    type1 = DB.LabelUtils.GetLabelForSpec(DB.SpecUtils.GetSpecTypeId(i))
                    dis = DeiziplinErmitteln(type1,Type)
                    allParaType.append([dis,type1,i])
                except:
                    pass
