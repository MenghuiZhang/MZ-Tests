...
class Familie(TemplateItemBase):
    ...
    def get_daten(self):
        self.typ = self.symbol.LookupParameter('Typenkommentare').AsString()
...
def get_Heizkoeper_IS():
    Dict = {}
    filter_ = DB.ElementMulticategoryFilter(List[DB.BuiltInCategory]([DB.BuiltInCategory.OST_DuctTerminal,]))
    ...
    Dict1 = {}
    for el in Families:
        if el.FamilyCategoryId.IntegerValue in [-2008013]:
