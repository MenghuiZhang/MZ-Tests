...
from eventhandler import ResizeAnsicht,ResetAnsicht,Luftauslass,ShowElement,HighLightElement,\
    VSR,DICT_MEP_ITEMSSOIRCE,VISIBLE,HIDDEN,Keyboard,Key,ChangeParameterWert,\
    RED,BLACK,UNDO,UPDATEMODEL,VERTEILEN,VERTEILEN_MEP,BERECHNEN,ResizeAnsicht1, UPDATEMODELALL
...
class MEPRaum_Uebersicht(forms.WPFWindow):
    def __init__(self):
        ...
        self.updateModelall = UPDATEMODELALL()
        self.resetAnsicht = ResetAnsicht()
        self.resizeAnsicht1 = ResizeAnsicht1()
        self._undo = UNDO()
        ...
        self.updateModelallEvent = ExternalEvent.Create(self.updateModelall)
        ...
        self.resizeAnsichtEvent1 = ExternalEvent.Create(self.resizeAnsicht1)
    ...
    def zeigraum1(self, sender, args):
        self.resizeAnsichtEvent1.Raise()
    ...
    def tnachtbetriebchanged(self, sender, args):
        try:
            self.mepraum.tiefenachtbetrieb = sender.IsChecked
            self.mepraum.Nachtbetrieb_Berechnen()
            self.Auswertung_MEP()
            self.grundinfo.Items.Refresh()
            self.anlageninfo.Items.Refresh()
    ...
    def VSR_checked_changed(self, sender, args):
        ...
        if Checked:
            ...
            self.lv_vsr_getrennt.Items.Refresh()
            self.lv_vsr1.Items.Refresh()
            self.lv_vsr1_getrennt.Items.Refresh()
            self.lv_vsr2.Items.Refresh()
            self.lv_vsr2_getrennt.Items.Refresh()
    ...
    def Auslass_Selected_Changed_getrennt(self, sender, args):
        if self.lv_auslass_getrennt.SelectedIndex != -1 and self.lv_auslass_getrennt.SelectedIndex != self.lv_auslass.SelectedIndex:
            self.highlightelement.elemId = self.lv_auslass_getrennt.SelectedItem.elemid
            self.highlightelementEvent.Raise()
            self.lv_vsr.SelectedIndex = -1
            self.lv_vsr_getrennt.SelectedIndex = -1
            self.lv_vsr1.SelectedIndex = -1
    ...
    def VSR_Selected_Changed_getrennt(self, sender, args):
        if self.lv_vsr_getrennt.SelectedIndex != -1 and self.lv_vsr_getrennt.SelectedIndex != self.lv_vsr.SelectedIndex:
            self.highlightelement.elemId = self.lv_vsr_getrennt.SelectedItem.elemid
            self.highlightelementEvent.Raise()
            self.lv_auslass.SelectedIndex = -1
            self.lv_auslass_getrennt.SelectedIndex = -1
            self.lv_vsr1.SelectedIndex = -1
    ...
    def VSR1_Selected_Changed(self, sender, args):
        if self.lv_vsr1.SelectedIndex != -1 and self.lv_vsr1_getrennt.SelectedIndex != self.lv_vsr1.SelectedIndex:
            self.highlightelement.elemId = self.lv_vsr1.SelectedItem.elemid
            self.highlightelementEvent.Raise()
            self.lv_auslass.SelectedIndex = -1
            self.lv_auslass_getrennt.SelectedIndex = -1
            self.lv_vsr.SelectedIndex = -1
    ...
    def VSR1_Selected_Changed_getrennt(self, sender, args):
        if self.lv_vsr1_getrennt.SelectedIndex != -1 and self.lv_vsr1_getrennt.SelectedIndex != self.lv_vsr1.SelectedIndex:
            self.highlightelement.elemId = self.lv_vsr1_getrennt.SelectedItem.elemid
            self.highlightelementEvent.Raise()
            self.lv_auslass.SelectedIndex = -1
            self.lv_auslass_getrennt.SelectedIndex = -1
            self.lv_vsr.SelectedIndex = -1
    ...
    def VSR2_Selected_Changed(self, sender, args):
        if self.lv_vsr2.SelectedIndex != -1 and self.lv_vsr2_getrennt.SelectedIndex != self.lv_vsr2.SelectedIndex:
            self.highlightelement.elemId = self.lv_vsr2.SelectedItem.elemid
            self.highlightelementEvent.Raise()
            self.lv_auslass.SelectedIndex = -1
            self.lv_auslass_getrennt.SelectedIndex = -1
            self.lv_vsr.SelectedIndex = -1
    ...
    def VSR2_Selected_Changed_getrennt(self, sender, args):
        if self.lv_vsr2_getrennt.SelectedIndex != -1 and self.lv_vsr2_getrennt.SelectedIndex != self.lv_vsr2.SelectedIndex:
            self.highlightelement.elemId = self.lv_vsr2_getrennt.SelectedItem.elemid
            self.highlightelementEvent.Raise()
            self.lv_auslass.SelectedIndex = -1
            self.lv_auslass_getrennt.SelectedIndex = -1
            self.lv_vsr.SelectedIndex = -1
    ...
    def update(self, sender, args):
        try:
            ...
            self.lv_vsr.Items.Refresh()
            self.lv_auslass_getrennt.Items.Refresh()
            self.lv_vsr_getrennt.Items.Refresh()
            self.lv_vsr1.Items.Refresh()
            self.lv_vsr1_getrennt.Items.Refresh()
            self.lv_vsr2.Items.Refresh()
    ...
    def updaterevitall(self, sender, args):
        try:
            self.updateModelallEvent.Raise()
        except:pass
...
mepWPF.updateModelall.GUI_MEPRaum = mepWPF
mepWPF.resizeAnsicht.GUI_MEPRaum = mepWPF
mepWPF.resizeAnsicht1.GUI_MEPRaum = mepWPF
