...
class MEPRaum:
    ...
    def zuluft_kuelleistung_berechnen(self):
        ...
        if self.Vol_zu and T_zu and T_raum:
            Kuelhlleistung = self.Vol_zu * 353 / (self.T_zu + 273) * 0.28 * (self.T_zu - self.T_raum)

        logger.info("Kühlleistung = {}".format(Kuelhlleistung))

        return Kuelhlleistung
...
output.print_table(
    ...
    title="Luftmengen",
    columns=['Nummer', 'Name', 'Vol_zu', 'ZUL Excel', 'T_raum',
