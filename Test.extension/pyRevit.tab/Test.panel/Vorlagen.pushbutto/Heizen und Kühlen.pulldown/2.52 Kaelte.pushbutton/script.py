...
__title__ = "2.52 K_KA_Schacht"
__doc__ = """Schacht von Lüftung Übernehmen, Kälte_Kühlleistung von MEP-Raum und Kühlleistung bzw. Wassermenge von Schacht berechnen"""
...
with forms.ProgressBar(title='{value}/{max_value} Raumliste',
    ...
    for Space in spaces_collector:
        ...
        umluft_vl = get_value(Space.LookupParameter("IGF_K_Schacht_KA-VL"))
        umluft_rl = get_value(Space.LookupParameter("IGF_K_Schacht_KA-RL"))
...
if forms.alert('Schächte von Lüftung übernehmen?', ok=False, yes=True, no=True):
    ...
    with forms.ProgressBar(title='{value}/{max_value} Schächte übernehmen',
        ...
        for Spaces in spaces_collector:
            ...
            KA_VL_Schacht = Spaces.LookupParameter("IGF_K_Schacht_KA-VL")
            KA_RL_Schacht = Spaces.LookupParameter("IGF_K_Schacht_KA-RL")
            if Zuluft_Schacht and Zuluft_Schacht != 'None':
                KA_VL_Schacht.Set(str(Zuluft_Schacht))
            if Abluft_Schacht and Abluft_Schacht != 'None':
                KA_RL_Schacht.Set(str(Abluft_Schacht))
...
class MEPRaum:
    ...
    def __init__(self, element_id):
        ...
        attr = [
            ...
            ['Schacht_VL', 'IGF_K_Schacht_KA-VL'],
            ['Schacht_RL', 'IGF_K_Schacht_KA-RL'],
            ['Wassermenge', 'IGF_K_KA_Wassermenge'],
            ['Temp_VL', 'IGF_K_KA-VL_Temp'],
            ['Temp_RL', 'IGF_K_KA-RL_Temp'],
        ]
...
# KA_Menge_Liste erstellen
KA_M_VL_Liste = []
KA_M_RL_Liste = []

# KA_Leistung_Liste erstellen
KA_L_VL_Liste = []
KA_L_RL_Liste = []
...
for i in range(len(Schacht)):
    KA_M_VL_Liste.append(0)
    KA_M_RL_Liste.append(0)
    KA_L_VL_Liste.append(0)
    KA_L_RL_Liste.append(0)
        ...
        for SPACE in spaces_collector:
            ...
            for Raum_Date in table_data:
                if Raum_Date[0] == Raum_Nr:
                    SPACE.LookupParameter('IGF_K_KA_Leistung').SetValueString(str(Raum_Date[7]))
        t1.Commit()


# KA_Leistung berechnen
with forms.ProgressBar(title='{value}/{max_value} Heizleistung von Schacht Berechnen',
    ...
    for space in spaces_collector:
        ...
        KA_V_S = get_value(space.LookupParameter('IGF_K_Schacht_KA-VL'))
        KA_R_S = get_value(space.LookupParameter('IGF_K_Schacht_KA-RL'))
        Wassermenge = get_value(space.LookupParameter('IGF_K_KA_Wassermenge'))
        Leistung = get_value(space.LookupParameter('IGF_K_KA_Leistung'))

        for ii in range(len(Schacht)):
            if KA_V_S == Schacht[ii]:
                KA_M_VL_Liste[ii] = Wassermenge + KA_M_VL_Liste[ii]
                KA_L_VL_Liste[ii] = Leistung + KA_L_VL_Liste[ii]
            if KA_R_S == Schacht[ii]:
                KA_M_RL_Liste[ii] = Wassermenge + KA_M_RL_Liste[ii]
                KA_L_RL_Liste[ii] = Leistung + KA_L_RL_Liste[ii]

table_data_Schacht = []
for Nr in range(len(Schacht)):
    table_data_Schacht.append([Schacht[Nr], KA_M_VL_Liste[Nr], KA_M_RL_Liste[Nr],KA_L_VL_Liste[Nr], KA_L_RL_Liste[Nr],])


output.print_table(
    table_data=table_data_Schacht,
    title="Schächte",
    columns=[ 'Schacht','KA-VL_Menge','KA-RL_Menge','KA-VL_Leistung','KA-RL_Leistung',  ]
)
            ...
            if Ins_Schacht:
                for num in range(len(Schacht)):
                    if Ins_Schacht_Name == Schacht[num]:
                        Schacht_V = SPACE.LookupParameter('IGF_K_Schacht_KA-VL_Menge')
                        Schacht_R = SPACE.LookupParameter('IGF_K_Schacht_KA-RL_Menge')
                        SchachtV = SPACE.LookupParameter('IGF_K_Schacht_KA-VL_Leistung')
                        SchachtR = SPACE.LookupParameter('IGF_K_Schacht_KA-RL_Leistung')
                        Schacht_V.SetValueString(str(KA_M_VL_Liste[num]))
                        Schacht_R.SetValueString(str(KA_M_RL_Liste[num]))
                        SchachtV.SetValueString(str(KA_L_VL_Liste[num]))
                        SchachtR.SetValueString(str(KA_L_RL_Liste[num]))
