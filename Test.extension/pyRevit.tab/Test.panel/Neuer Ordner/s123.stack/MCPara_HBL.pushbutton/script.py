...
def AddFamilyParam1():
        ...
        for defi in list_definition:
            try:
                ...
                if para.Definition.Name == 'MC Height Instance':
                    ...
                    try:
                        m_familyMgr.SetFormula(para,"MC_R1*2")
                    except:
                        try:m_familyMgr.SetFormula(para,'Durchmesser Luftkanal')
                            ...
                            try:m_familyMgr.SetFormula(para,'D')
                                ...
                                try:m_familyMgr.SetFormula(para,'Durchmesser')
                                    ...
                                    try:m_familyMgr.SetFormula(para,'Luftkanaldurchmesser')
                                    except:pass
            except:pass
       # fdoc.Regenerate()
        trans1.ForceCloseTransaction()
    ...
    try:
        ...
        if rpw.revit.doc.Title.find('.rfa') != -1:
            rpw.revit.doc.SaveAs('R:\\Familien\\420_H\\_RUB_NA\\'+rpw.revit.doc.Title)
        else:
            rpw.revit.doc.SaveAs('R:\\Familien\\420_H\\_RUB_NA\\'+rpw.revit.doc.Title+'.rfa')
    except:
        try:
            rpw.revit.doc.Save()
        except Exception as e:
