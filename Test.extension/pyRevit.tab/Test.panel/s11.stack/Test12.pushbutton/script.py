...
__title__ = "3.90 Vorgabe von Raumluftbilanz"

__doc__ = """
Vorgabe von Raumluftbilanz

update: Raumsuche für Laboreingabe
...
class GUI(forms.WPFWindow):
    def __init__(self):
        ...
        self.Dict_MEP = Dict_MEPRaum
        self.IS_Schacht_Neu = IS_Schacht_Neu
        ...
        self.Raumdatensource.ItemsSource = sorted(self.Dict_MEP.keys())
    ...
    def RaumLaborauswahl(self, sender, args):
        self.externaleventhandler.ExcuseApp = self.externaleventhandler.MEP_Labor
        self.externalevent.Raise()
    
    def auswahl1(self, sender, args):
        self.externaleventhandler.ExcuseApp = self.externaleventhandler.SelectRaum1
        self.externalevent.Raise()
    
    def auswahl2(self, sender, args):
        self.externaleventhandler.ExcuseApp = self.externaleventhandler.SelectRaum2
        self.externalevent.Raise()
    ...
    def schachtfaktorschreiben(self, sender, args):
      
        self.externaleventhandler.ExcuseApp = self.externaleventhandler.schachtfaktorschreiben
        self.externalevent.Raise()


    def fakt_textchanged(self,sender,e):
        
        if self.anl_schacht.SelectedIndex != -1:
            text = sender.Text
            item = sender.DataContext
            if self.anl_schacht.SelectedItem is not None:
                if item in self.anl_schacht.SelectedItems:
                    
                    for el in self.anl_schacht.SelectedItems:
                        el.faktor = text
    ...
    def Textinput4(self, sender, args):
        if sender.Text not in [None,'']:
            if sender.Text.find('.') != -1:
                if args.Text == '.':args.Handled = True
                return
        try:
            args.Handled = self.regex4.IsMatch(args.Text)
        except:
            args.Handled = True
