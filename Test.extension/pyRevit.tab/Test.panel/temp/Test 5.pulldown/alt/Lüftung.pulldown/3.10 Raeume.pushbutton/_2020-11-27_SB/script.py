...
class MEPRaum:
    def __init__(self, element_id):
        ...
        self.abluft_min_incl_Summe24h = self.abluft_min_berechnen_mit()
    ...
    def abluft_min_berechnen_mit(self):
        abluft_mit = 0
        ...
        if int(self.kez) in [
            ...
            abluft_mit = self.zuluft_min - self.raum_druckstufe

        elif int(self.kez) == berechnung_nach['nurABMa']:
            abluft_mit = self.vol_faktor
        ...
        logger.info("ABL_ZR = {}".format(abluft_mit))

        abluft_mit = abluft_mit + self.ueberstroemung - sum(stroemt_ueber)

        logger.info("ABL_M = {}".format(abluft_mit))
    ...
    def abluft_min_berechnen(self):
        ...
        abluft = abluft + self.ueberstroemung - sum(stroemt_ueber) - self.ABL_24h
    ...
    def werte_schreiben(self):
        ...
        wert_schreiben("IGF_RLT_AbluftminRaum", self.abluft_min_incl_Summe24h)
        wert_schreiben("Angegebener Zuluftstrom", self.zuluft_min)
        wert_schreiben("Angegebener Abluftluftstrom", self.abluft_min_incl_Summe24h)
    ...
    def table_row(self):
        ...
        return [
            ...
            self.ebene,
            self.zuluft_min,
            self.__get_element_attr("IGF_RLT_ZuluftminRaum"),
            self.ABL_24h,
            self.abluft_min_incl_Summe24h,
            self.abluft_min,
            self.__get_element_attr("IGF_RLT_AbluftminRaum"),
            self.abluft_labor_24h,
            self.__get_element_attr("IGF_RLT_AbluftminSummeLabor24h"),
            self.nb_dauer,
            self.zu_nacht,
            self.ab_nacht,
...
output.print_table(
    ...
    title="Luftmengen",
    columns=['Nummer', 'Name', 'Ebene', 'ZUL pyRevit', 'ZUL Excel', 'ABL_24h',
             'ABL_mitSumme24h','ABL pyRevit', 'ABL Excel', 'Labor pyRevit',
             'Labor Excel', 'nb Dauer', 'nb zu', 'nb ab','tief nb Dauer',
             'tief nb zu','tief nb ab']
)
...
#daten = table_data[:]
#colu=['Nummer', 'Name', 'Ebene', 'ZUL pyRevit', 'ZUL Excel', 'ABL_24h',
#             'ABL_mitSumme24h','ABL pyRevit', 'ABL Excel', 'Labor pyRevit',
#             'Labor Excel', 'nb Dauer', 'nb zu', 'nb ab','tief nb Dauer',
#             'tief nb zu','tief nb ab']
#daten.insert(0,colu)

#workbook = xlsxwriter.Workbook('C:\\Users\\werksstudent\\Desktop\\0.xlsx')
