...
if forms.alert('Kenngut in Modell schreiben?', ok=False, yes=True, no=True):
    with forms.ProgressBar(title='{value}/{max_value} Kenngut Beschriften',
                           cancellable=True, step=10) as pb2:

        n_1 = 0

        with rpw.db.Transaction("Kenngut Beschriften"):
            for Luftkanalzubehoer in Luftkanalzubehoer_Liste:
                if pb2.cancelled:
                    script.exit()
                n_1 += 1
                pb2.update_progress(n_1, len(ducts))

                Luftkanalzubehoer.werte_schreiben()
