...
__title__ = "4.20 Laborwasser"
__doc__ = """Schacht von Lüftung Übernehmen und Laborwasser von Schacht berechnen"""
...
if forms.alert('Schächte von Lüftung übernehmen?', ok=False, yes=True, no=True):
    ...
    with forms.ProgressBar(title='{value}/{max_value} Schächte übernehmen',
        ...
        for Spaces in spaces_collector:
            ...
            Schacht_LWK = Spaces.LookupParameter("IGF_S_Schacht_Laborwasser_kalt")
            Schacht_LWW = Spaces.LookupParameter("IGF_S_Schacht_Laborwasser_warm")
            Schacht_LWZ = Spaces.LookupParameter("IGF_S_Schacht_Laborwasser_Zirkulation")
            if Zuluft_Schacht and Zuluft_Schacht != 'None':
                Schacht_LWK.Set(str(Zuluft_Schacht))
                Schacht_LWW.Set(str(Zuluft_Schacht))
            if Abluft_Schacht and Abluft_Schacht != 'None':
                Schacht_LWZ.Set(str(Abluft_Schacht))
...
class MEPRaum:
    ...
    def __init__(self, element_id):
        ...
        attr = [
            ['name', 'Name'],
            ['nummer', 'Nummer'],
            ['LWC_Menge', 'IGF_S_Laborwasser-kalt'],
            ['LWC_VL_Menge', 'IGF_S_Schacht_Laborwasser_kalt_VL'],
            ['LWC_RL_Menge', 'IGF_S_Schacht_Laborwasser_kalt_RL'],
            ['LWH_Menge', 'IGF_S_Laborwasser-warm'],
            ['LWH_C_Menge', 'IGF_S_Laborwasser-Zirkulation'],
            ['Schacht_LWC', 'IGF_S_Schacht_Laborwasser_kalt'],
            ['Schacht_LWH', 'IGF_S_Schacht_Laborwasser_warm'],
            ['Schacht_LWH_C', 'IGF_S_Schacht_Laborwasser_Zirkulation'],
        ]
...
output.print_table(
    ...
    title="Mep Räume",
    columns=['Nummer', 'Name', 'Schacht_LWK', 'LWK in MEP Raum', 'LWK_VL(Ringleitung)',
             'LWK_RL(Ringleitung)', 'Schacht_LWW', 'LWW in MEP Raum','Schacht_LWZ',
             'LWZ in MEP Raum',]
)
# Laborwasser Liste erstellen
LWK_Liste = []
LWW_Liste = []
LWZ_Liste = []
    ...
    for data in table_data:
        ...
        for ii in range(len(Schacht)):
            if data[2] == Schacht[ii]:
                LWK_Liste[ii] = data[3] + LWK_Liste[ii]
                LWW_Liste[ii] = data[7] + LWK_Liste[ii]
            if data[8] == Schacht[ii]:
                LWZ_Liste[ii] = data[9] + LWZ_Liste[ii]


table_data_Schacht = []
for Nr in range(len(Schacht)):
    table_data_Schacht.append([Schacht[Nr],LWK_Liste[Nr],LWW_Liste[Nr],LWZ_Liste[Nr]])


output.print_table(
    table_data=table_data_Schacht,
    title="Schächte",
    columns=[ 'Schacht','Laborwasser_Kalt', 'Laborwasser_Warm','Laborwasser_Zirkulation' ]
)
            ...
            if Ins_Schacht:
                for Daten in table_data_Schacht:
                    if Ins_Schacht_Name == Daten[0]:
                        schacht_LWK = SPACE.LookupParameter('IGF_S_Schacht_Laborwassermenge_kalt')
                        schacht_LWK.SetValueString(str(Daten[1]))
                        schacht_LWW = SPACE.LookupParameter('IGF_S_Schacht_Laborwassermenge_warm')
                        schacht_LWW.SetValueString(str(Daten[2]))
                        schacht_LWZ = SPACE.LookupParameter('IGF_S_Schacht_Laborwassermenge_Zirkulation')
                        schacht_LWZ.SetValueString(str(Daten[3]))
