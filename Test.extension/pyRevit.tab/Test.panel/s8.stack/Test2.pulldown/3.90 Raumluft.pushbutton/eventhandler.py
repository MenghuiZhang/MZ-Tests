...
class ListeExternalEvent(IExternalEventHandler):
    ...
    def UpdateRaumInfo_Labor(self,uiapp):
        ...
        for Param in self.GUI.IS_Laboranschluss:
            value = el.LookupParameter(Param.parametername).AsString()
            if value:
                try:
    ...
    def SchreibenLaborDetail_Anschluss(self,uiapp):
        ...
        colls = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_DuctTerminal).WhereElementIsNotElementType().WherePasses(fam_filter).WherePasses(filter1).ToElements()
        colls_ = {}
