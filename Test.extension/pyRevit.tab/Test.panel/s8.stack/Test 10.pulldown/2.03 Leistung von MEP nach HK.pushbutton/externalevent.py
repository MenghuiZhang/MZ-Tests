...
class HeizkoerperInstance(TemplateItemBase):
    ...
    def get_uebertemp(self):
        if self.vorlauf == 0 or self.ruecklauf == 0:
            self.uebertemp = 0
            return
                ...
                if conn.IsConnected:
                    ...
                    for ref in allrefs:
                        if ref.Owner.Category.Name == 'Rohrzubehör':
                            if ref.Owner.Symbol.FamilyName.upper().find('ANSCHLUSSSET') != -1:
                                zube = ref.Owner
                                break
            if not zube:
                return
            for conn in zube.MEPModel.ConnectorManager.Connectors:
                if conn.IsConnected:
                    MEPSystem = conn.MEPSystem
    ...
    def GetRaum(self):
        try:
            self.raum = self.elem.Space[self.phase].Id.ToString()
        except:
...
class MEP_Raum:
    def __init__(self,elemid,HK_Bauteile):
        ...
        self.LeistungInDouble = self.get_value(self.elem.LookupParameter('IGF_H_HeizlastGesamt'))
        self.Leistung = self.elem.LookupParameter('IGF_H_HeizlastGesamt').AsValueString()
        self.raumslavevon = self.elem.LookupParameter('IGF_A_Gehört_zu_Raum').AsString()
        self.HK_Update()
        self.Auswerten()
    ...
    for el in spaces:
        try:
            ...
            if el.Id.ToString() in Dict_MEP.keys():
                mepraum = MEP_Raum(el.Id,ObservableCollection[HeizkoerperInstance](Dict_MEP[el.Id.ToString()]))
                if mepraum.raumslavevon:
                    if mepraum.raumslavevon not in  Dict.keys():
                        Dict[mepraum.raumslavevon] = [mepraum]
                    else:
                        Dict[mepraum.raumslavevon].append(mepraum)

                if name not in Dict_MEP_Itemssource.keys():Dict_MEP_Itemssource[name] = mepraum
                else:
                    n = 1
                    while ((name+' '+str(n)) in Dict_MEP_Itemssource.keys()):
                        n+=1
                        if n > 50:
                            break
                    Dict_MEP_Itemssource[name+' '+str(n)] = mepraum
        except Exception as e:
            logger.error(e)
        if len(Dict.keys()) > 0:
            for el in Dict_MEP_Itemssource.values():
                if el.elemid.ToString() in Dict.keys():
                    mepraums = Dict[el.elemid.ToString()]
                else:
                    continue
                    
                liste_neu = []
                liste_neu.extend(Dict_MEP[el.elemid.ToString()])
                for mep in mepraums:
                    liste_neu.extend(Dict_MEP[mep.elemid.ToString()])
                is_liste = ObservableCollection[HeizkoerperInstance](liste_neu)
                el.HK_Bauteile = is_liste
                for mep in mepraums: mep.HK_Bauteile = is_liste
