...
for sheet in book1.Worksheets:
    with forms.ProgressBar(title="{value}/{max_value} Zeile --- Excel lesen",cancellable=True, step=10) as pb:
        ...
        for row in range(2, rows + 1):
            ...
            cutlayername = sheet.Cells[row, 4].Value2
            cutlayerfarbe = sheet.Cells[row, 5].Value2
            systemtyp_dict[name] = [layername,layerfarbe,cutlayername,cutlayerfarbe]
    ...
    if Categoryname == 'Systemtyp':
        if name != 'Systemtyp':
            if name in systemtyp_dict.keys():
                ...
                layinfo.CutLayerName = systemtyp_dict[name][2]
                if systemtyp_dict[name][3]:
                    layinfo.CutColorNumber = int(systemtyp_dict[name][3])
