...
__title__ = "6.VE Wasser"
__doc__ = """Schacht von Lüftung Übernehmen und VE Wasser berechnen"""
...
with forms.ProgressBar(title='{value}/{max_value} Raumliste',
    ...
    #    vewasser_zu = get_value(Space.LookupParameter("TGA_RLT_Schacht_zu_VEWasser"))
    #    vewasser_ab = get_value(Space.LookupParameter("TGA_RLT_Schacht_ab_VEWasser"))
        tabel_Date_MEP.append([nummer,name,luft_zu,luft_ab])
#        tabel_Date_MEP.append([nummer,name,luft_zu,luft_ab,vewasser_zu,vewasser_ab])
        if ins_schacht:
            schacht.append([nummer,name,install_schacht_name,luft_zu,luft_ab])
#            schacht.append([nummer,name,install_schacht_name,luft_zu,luft_ab,vewasser_zu,vewasser_ab])
...
#    columns=['nummer', 'name',  'Zuluft_Schacht','Abluft_Schacht',
#             'Zu_VEWasser_Schacht','Ab_VEWasser_Schacht']
)
...
#            Zu_VEWasser_Schacht = Spaces.LookupParameter("TGA_RLT_Schacht_zu_VEWasser")
#            Ab_VEWasser_Schacht = Spaces.LookupParameter("TGA_RLT_Schacht_ab_VEWasser")
#            Zu_VEWasser_Schacht.Set(str(Zuluft_Schacht))
#            Ab_VEWasser_Schacht.Set(str(Abluft_Schacht))
...
class MEPRaum:
    ...
    def __init__(self, element_id):
        ...
        attr = [
            ...
            ['Zu_Schacht_Nr', 'TGA_RLT_Schacht_zu_VEWasser'],
            ['Zu', 'IGF_S_zu_VEWasser'],
            ['Ab_Schacht_Nr', 'TGA_RLT_Schacht_ab_VEWasser'],
            ['Ab', 'IGF_S_ab_VEWasser']
        ]
...
#    Zuwasser_Liste.append(0)
#    Abwasser_Liste.append(0)
...
#        Zu_Schacht_Nr = get_value(space.LookupParameter('TGA_RLT_Schacht_zu_VEWasser'))
#        Zu_VEWasser = get_value(space.LookupParameter('IGF_S_zu_VEWasser'))
#        Ab_Schacht_Nr = get_value(space.LookupParameter('TGA_RLT_Schacht_ab_VEWasser'))
#        Ab_VEwasser = get_value(space.LookupParameter('IGF_S_ab_VEWasser'))

#        for ii in range(len(Schacht)):
#            if Zu_Schacht_Nr == Schacht[ii]:
#                Zuwasser_Liste[ii] = Zu_VEWasser + Zuwasser_Liste[ii]
#        for jj in range(len(Schacht)):
#            if Ab_Schacht_Nr == Schacht[jj]:
#                Abwasser_Liste[jj] = Ab_VEwasser + Abwasser_Liste[jj]


#table_data_Schacht = []
#for Nr in range(len(Schacht)):
#    table_data_Schacht.append([Schacht[Nr],Zuwasser_Liste[Nr],Abwasser_Liste[Nr]])
...
#             'Zu_VEWasser', 'Ab_Schacht_Nr', 'Ab_VEwasser' ]
#)

#output.print_table(
#    table_data=table_data_Schacht,
#    title="Gasemengen",
#    columns=[ 'Schacht','Zu_VEWasser', 'Ab_VEwasser' ]
#)
...
#                        zuschacht = SPACE.LookupParameter('IGF_S_SchachtzuVEWasserMenge')
#                        zuschacht.SetValueString(str(Zuwasser_Liste[num]))
#                        abschacht = SPACE.LookupParameter('IGF_S_SchachtAbVEWasserMenge')
#                        abschacht.SetValueString(str(Abwasser_Liste[num]))
