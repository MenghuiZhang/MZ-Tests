...
class MEPRaum:
    def __init__(self, element_id):
        ...
        attr = [
            ...
            ['nacht','IGF_RLT_Nachtbetrieb'],
            ['nbv','IGF_RLT_NachtbetriebVon'],
            ['nbb','IGF_RLT_NachtbetriebBis'],
            ['nbd','IGF_RLT_NachtbetriebDauer'],
            ['nbl','IGF_RLT_NachtbetriebLW']
        ]
        ...
        self.nbd = self.nb_dauer_berechnen()
        self.zun = self.zuluft_nacht_berechnen()
        self.abn = self.abluft_nacht_berechnen()
    ...
    def nb_dauer_berechnen(self):
        nbd = self.nbv - self.nbb

        logger.info("Nachtbetrieb_Dauer = {}".format(nbd))
        return nbd

    def abluft_nacht_berechnen(self):
        abn = 0

            if int(self.nacht) ==  Nacht_Berechnung_nach['aus']:
                abn = 0
                ...
                if int(self.kez) in [
                    ...
                    abn = self.zun - self.raum_druckstufe
                ...
                logger.info("strömt_über raus: {}".format(sum(stroemt_ueber)))
                logger.info("strömt_über rein: {}".format(self.ueberstroemung))
                logger.info("ABL_NACHT_ZR = {}".format(abn))

                abn = abn + self.ueberstroemung - sum(stroemt_ueber)

          logger.info("ABL_NACHT = {}".format(abn))
    ...
    def zuluft_nacht_berechnen(self):
        ...
        if int(self.nacht) ==  Nacht_Berechnung_nach['aus']:
           ...
           zun = self.nbl *  self.volumen

           if self.abluft_labor_24h > zun:
              logger.info(
                "{} m/h3 Labor und 24h Abluft berücksichtigt".format(self.abluft_labor_24h))

              zun = self.abluft_labor_24h

           if self.raum_druckstufe > 0:
              logger.info(
                  "{} - Druckstufe berücksichtigt".format(self.raum_druckstufe))

              zun = zun + self.raum_druckstufe

    logger.info("ZUL_Nacht = {}".format(zun))
    ...
    def table_row(self):
        ...
        return [
            ...
            self.nbd,
            self.__get_element_attr("NB_Dauer"),
            self.zun,
            self.__get_element_attr("ZuluftNacht"),
            self.abn,
            self.__get_element_attr("AbluftNacht"),
        ]
...
output.print_table(
    ...
    columns=['Nummer', 'Name', 'ZUL pyRevit',
             'ZUL Excel', 'ABL pyRevit', 'ABL Excel', 'Labor pyRevit', 'Labor Excel']
)
