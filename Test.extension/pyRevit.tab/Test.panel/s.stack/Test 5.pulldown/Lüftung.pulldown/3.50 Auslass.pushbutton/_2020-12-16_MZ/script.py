        ...
        def summe(liste,kz):
            ...
            for ele in liste:
                for i in range(1,len(summe)):
                    Summe[i] = Summe[i] + ele[i+4]
            return Summe

        raum_auslass = [Raum_Nr,Zu,Ab,Ab_24h,Ueber,auslass[len(auslass)],
                        summe(Zu,'Zuluft_Summe'),summe(Ab,'Abluft_Summe'),
                        summe(Ab_24h,'Ab24h_Summe'),summe(Ueber,'Über_Summe')]
...
def final_liste(liste):
    ...
    for item in liste:
        ...
        output.print_table(
            table_data = final[1:],
            title = final[0],
            columns=['Vol', 'Vol_min','Vol_max', 'Vol_Nacht',
                     'Vol_Tiefnacht']
                     )
    return Final
...
Final_Liste = final_liste(Raum_Durchlass)
