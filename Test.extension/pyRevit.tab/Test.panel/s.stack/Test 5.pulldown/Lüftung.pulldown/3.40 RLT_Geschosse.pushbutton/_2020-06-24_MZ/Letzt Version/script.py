...
for Space in spaces_collector:
    ...
    Ebenen = get_value(Space.LookupParameter('Ebene'))
    ...
    for aa in range(len(Schacht_gesamt)):
        S = Schacht_gesamt[aa]
        if Zu_Schacht == SchachtListe[aa]:
            S_zu = S[0]
            for bb in range(len(EbeneListe)):
                if Ebenen == EbeneListe[bb]:
                    E_zu = S_zu[bb]
        ...
        if Ab_Schacht == SchachtListe[aa]:
            S_ab = S[1]
            for dd in range(len(EbeneListe)):
                if Ebenen == EbeneListe[dd]:
                    E_ab = S_ab[dd]
        ...
        if Ab24h_Schacht == SchachtListe[aa]:
            S_ab24h = S[2]
            for ff in range(len(EbeneListe)):
                if Ebenen == EbeneListe[ff]:
                    E_ab24h = S_ab24h[ff]
...
for Spaces in spaces_collector:
    ...
    if Ins_Schacht:
        Luft_zu = 'Luftverteilung: '
        Luft_ab = 'Luftverteilung: '
        Luft_ab = 'Luftverteilung: '
        for num in range(len(SchachtListe)):
            if Ins_Schacht_Name == SchachtListe[num]:
                Sch_zu = Schacht_gesamt[num][0]
                Sch_ab = Schacht_gesamt[num][1]
                Sch_ab24h = Schacht_gesamt[num][2]
                for aaa in range(len(EbeneListe)):
                    ebene = EbeneListe[aaa]
                    ebe_zu = Sch_zu[aaa]
                    ebe_ab = Sch_ab[aaa]
                    ...
                    for bbb in range(len(ZuAnlNrListe)):
                        if ebe_zu[bbb] > 0:
                            Luft_zu = Luft_zu + 'Anlage' + str(ZuAnlNrListe[bbb]) + '=' + str(ebe_zu[bbb]) + ' m3/h,'

                    if any(ebe_ab):
                        Luft_ab = Luft_ab + str(ebene) +': '
                    for ccc in range(len(AbAnlNrListe)):
                        if ebe_ab[ccc] > 0:
                            Luft_ab = Luft_ab + 'Anlage' + str(AbAnlNrListe[ccc]) + '=' + str(ebe_ab[ccc]) + ' m3/h,'


        logger.info("{}\t{}".format(Ins_Schacht_Name, Luft_zu))
        logger.info(30 * "=")
...
logger.info("{}".format(Schacht_gesamt))
