...
class MEPRaum_Uebersicht(forms.WPFWindow):
    ...
    def Setkey(self, sender, args):   
        if ((args.Key >= self.Key.D0 and args.Key <= self.Key.D9) or (args.Key >= self.Key.NumPad0 and args.Key <= self.Key.NumPad9) \
            or args.Key == self.Key.Delete or args.Key == self.Key.Back or args.Key == self.Key.Tab or args.Key == self.Key.Enter or args.Key == self.Key.OemMinus or args.Key == self.Key.Subtract):
            args.Handled = False
