...
class Familie(TemplateItemBase):
    def __init__(self,Familie,elems):
        ...
        self.elems = elems
        self.symbol = doc.GetElement(DB.ElementId((int(self.elems[0])))).Symbol
        try:self.get_daten()
        except:pass
...
def get_Heizkoeper_IS():
    ...
    for fam in sorted(Dict.keys()):
        AUSWAHL_HEIZKOERPER_IS.Add(Familie(fam,Dict[fam]))
...
class Familienauswahl(forms.WPFWindow):
    def __init__(self):
        ...
        self.temp_coll0 = ObservableCollection[Familie]()
    ...
    def nurchecked(self,sender,e):
        self.checkbox1.IsChecked = True
        self.checkbox2.IsChecked = True
        for el in self.HLS_IS:
            if el.checked:
                el.visibility = 1
            else:
                el.visibility = 0
        self.lv_HK1.Items.Refresh()
        self.lv_HK.Items.Refresh()
    
    def allebauteile(self,sender,e):
        for el in self.HLS_IS:
          
            el.visibility = 1
        self.checkbox1.IsChecked = False
        self.checkbox2.IsChecked = False
        
        self.lv_HK1.Items.Refresh()
        self.lv_HK.Items.Refresh()
    
    def textchanged0(self,sender,e):
        self.textchanged(sender,self.lv_HK,self.temp_coll0)
...
class HeizkoerperInstance(TemplateItemBase):
    ...
    def farbe(self):
        if self.Heizleistung > self.Nennleistung:
            self.fore = RED
        else:
            self.fore = BLACK
