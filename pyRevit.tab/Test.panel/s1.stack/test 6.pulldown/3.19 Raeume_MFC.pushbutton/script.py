...
class MEPRaum:
    def __init__(self, element_id):
        ...
        self.flaeche = get_value(self.element.LookupParameter('Fläche'))
        self.volumen = get_value(self.element.LookupParameter('Volumen'))
        self.personen = get_value(self.element.LookupParameter('Personenzahl'))
        self.vol_faktor = get_value(self.element.LookupParameter('TGA_RLT_VolumenstromProFaktor'))
        ...
        self.nachtbetrieb = get_value(self.element.LookupParameter('IGF_RLT_Nachtbetrieb'))
        self.NB_Von = get_value(self.element.LookupParameter('IGF_RLT_NachtbetriebVon'))
        self.NB_Bis = get_value(self.element.LookupParameter('IGF_RLT_NachtbetriebBis'))
        self.NB_LW = get_value(self.element.LookupParameter('IGF_RLT_NachtbetriebLW'))
        self.tiefernachtbetrieb = get_value(self.element.LookupParameter('IGF_RLT_TieferNachtbetrieb'))
        self.T_NB_Von = get_value(self.element.LookupParameter('IGF_RLT_TieferNachtbetriebVon'))
        self.T_NB_Bis = get_value(self.element.LookupParameter('IGF_RLT_TieferNachtbetriebBis'))
        self.T_NB_LW = get_value(self.element.LookupParameter('IGF_RLT_TieferNachtbetriebLW'))
        ...
        self.flaeche = round(self.flaeche+0.5, 0)
        self.ebene = self.element.LookupParameter('Ebene').AsValueString()
        ...
        self.zu_nacht = 0
        self.ab_nacht = 0
        self.ab_nacht_ohne24h = 0
        self.tiefer_nb_dauer = 0
        self.tiefer_zu_nacht = 0
        self.tiefer_ab_nacht = 0
        self.tiefer_ab_nacht_ohne24h = 0

        self.IGF_Druckstufe = 0
        self.IGF_Legende = ''
    ...
    def __lab_abl_24h_druckstufe_ueber_pruefen(self, zuluft):
        if self.abluft_labor_24h  > zuluft:
            logger.info("{} m/h3 Labor und 24h Abluft berücksichtigt".format(self.abluft_labor_24h))
    ...
    def Tagesbetrieb_Berechnen(self):
        if self.flaeche == 0:
            logger.error("Die Fläche des Raumes {} {} ist 0 m²".format(
                self.nummer, self.name))
            return
        if self.kez == berechnung_nach["nurZU_Fläche"]:
            logger.info("Berechnung nach Fläche. Nur Zuluft")
            self.zuluft_min = luft_round(self.flaeche * self.vol_faktor) + self.raum_druckstufe
            if self.abluft_labor_24h > 0:
                logger.error('Berechnungsprinzip von Raum {}-{} ist Falsch, da Laborabluft/24h Abluft vorhanden.'.format(self.nummer,self.name))
            abweichung = self.ueberstroemungAus - self.zuluft_min - self.ueberstroemungIn - self.ueberstroemung2 + self.ueberAusMa + self.abluft_labor_24h
            if abweichung > 0:
                self.hinweis = "Abluft durch Überströmung | Abweichung: +{} m³/h".format(int(abweichung))
            elif abweichung < 0:
                self.hinweis = "Achtung: Nur Zuluft, aber die Überströmung ist kleiner als die Zuluft. Abweichung: {} m³/h".format(int(abweichung))
            self.zuluft_min += abweichung
            self.angezuluft = self.zuluft_min
            self.angeabluft = self.angezuluft
        
        elif self.kez == berechnung_nach["nurZU_Luftwechsel"]:
            logger.info("Berechnung nach Luftwechsel. Nur Zuluft")
            self.zuluft_min = luft_round(self.volumen * self.vol_faktor) + self.raum_druckstufe
            if self.abluft_labor_24h > 0:
                logger.error('Berechnungsprinzip von Raum {}-{} ist Falsch, da Laborabluft/24h Abluft vorhanden.'.format(self.nummer,self.name))
            abweichung = self.ueberstroemungAus - self.zuluft_min - self.ueberstroemungIn - self.ueberstroemung2 + self.ueberAusMa + self.abluft_labor_24h
            if abweichung > 0:
            ...
            self.zuluft_min = luft_round(self.personen * self.vol_faktor) + self.raum_druckstufe
            if self.abluft_labor_24h > 0:
                logger.error('Berechnungsprinzip von Raum {}-{} ist Falsch, da Laborabluft/24h Abluft vorhanden.'.format(self.nummer,self.name))
            abweichung = self.ueberstroemungAus - self.zuluft_min - self.ueberstroemungIn - self.ueberstroemung2 + self.ueberAusMa + self.abluft_labor_24h
            if abweichung > 0:
            ...
            self.angezuluft = luft_round(self.flaeche * self.vol_faktor) + self.raum_druckstufe
            abweichung = self.angezuluft - self.ueberstroemungIn - self.ueberstroemung2
            if abweichung != 0:
                self.hinweis = "Zuluft durch Überströmung | Abweichung: {} m3/h".format(int(abweichung))
            abweichung2 = self.ueberstroemungIn + self.ueberstroemung2 - self.ueberstroemungAus - self.ueberAusMa - self.abluft_labor_24h - self.raum_druckstufe
            if abweichung2 < 0:
                self.angezuluft -= abweichung2
                self.abluft_min = self.abluft_labor_24h
                self.angeabluft = self.angezuluft
                self.abluft_ohne_24h = self.abluft_min - self.ABL_24h
                self.abluft_ges = self.abluft_min + self.ueberstroemungAus + self.ueberAusMa
            else:
                self.abluft_min = self.abluft_labor_24h + abweichung2
                self.angeabluft = self.angezuluft
                self.abluft_ohne_24h = self.abluft_min - self.ABL_24h
                self.abluft_ges = self.abluft_min + self.ueberstroemungAus + self.ueberAusMa
            ...
            self.angezuluft = luft_round(self.volumen * self.vol_faktor) + self.raum_druckstufe
            abweichung = self.angezuluft - self.ueberstroemungIn - self.ueberstroemung2
            if abweichung != 0:
                self.hinweis = "Zuluft durch Überströmung | Abweichung: {} m3/h".format(int(abweichung))
            abweichung2 = self.ueberstroemungIn + self.ueberstroemung2 - self.ueberstroemungAus - self.ueberAusMa - self.abluft_labor_24h - self.raum_druckstufe
            ...
            self.angezuluft = luft_round(self.personen * self.vol_faktor) + self.raum_druckstufe
            abweichung = self.angezuluft - self.ueberstroemungIn - self.ueberstroemung2
            if abweichung != 0:
                self.hinweis = "Zuluft durch Überströmung | Abweichung: {} m3/h".format(int(abweichung))
            abweichung2 = self.ueberstroemungIn + self.ueberstroemung2 - self.ueberstroemungAus - self.ueberAusMa - self.abluft_labor_24h - self.raum_druckstufe
            ...
            self.zuluft_min = luft_round(self.flaeche * self.vol_faktor) + self.raum_druckstufe
            self.angezuluft = self.zuluft_min
            abweichung = self.zuluft_min + self.ueberstroemungIn + self.ueberstroemung2 - self.ueberstroemungAus - self.ueberAusMa - self.abluft_labor_24h - self.raum_druckstufe
            if abweichung < 0:
                self.zuluft_min -= abweichung
                ...
                self.abluft_min = self.abluft_labor_24h + abweichung
                self.angeabluft = self.angezuluft
                self.abluft_ohne_24h = self.abluft_min - self.ABL_24h
                self.abluft_ges = self.abluft_min + self.ueberstroemungAus + self.ueberAusMa
            ...
            self.zuluft_min = luft_round(self.vol_faktor) + self.raum_druckstufe
            self.angezuluft = self.zuluft_min
            abweichung = self.zuluft_min + self.ueberstroemungIn + self.ueberstroemung2 - self.ueberstroemungAus - self.ueberAusMa - self.abluft_labor_24h - self.raum_druckstufe
            if abweichung < 0:
                self.zuluft_min -= abweichung
            ...
            self.angezuluft = luft_round(self.vol_faktor) + self.raum_druckstufe
            abweichung = self.angezuluft - self.ueberstroemungIn - self.ueberstroemung2
            if abweichung != 0:
                self.hinweis = "Zuluft durch Überströmung | Abweichung: {} m3/h".format(int(abweichung))
            abweichung2 = self.ueberstroemungIn + self.ueberstroemung2 - self.ueberstroemungAus - self.ueberAusMa - self.abluft_labor_24h - self.raum_druckstufe
    ...
    def Nachtbetrieb_Berechnen(self):
        if self.nachtbetrieb:
            self.nb_dauer = self.NB_Bis - self.NB_Von + 24.00
            if self.kez == berechnung_nach["Fläche"] or self.kez == berechnung_nach["Luftwechsel"] or self.kez == berechnung_nach["Person"] or self.kez == berechnung_nach["manuell"]:
                self.zu_nacht = luft_round(self.NB_LW * self.volumen) + self.raum_druckstufe
        ...
        #     if abweichung < 0:
        #         self.zu_nacht -= abweichung
        #         self.ab_nacht = self.abluft_labor_24h
        #         self.ab_nacht_ohne24h = self.abluft_labor_24h - self.ABL_24h
        #     else:
        #         self.ab_nacht = self.abluft_labor_24h + abweichung
        #         self.ab_nacht_ohne24h = self.abluft_labor_24h - self.ABL_24h

        if self.tiefernachtbetrieb:
            self.tiefer_nb_dauer = self.T_NB_Bis - self.T_NB_Von + 24.00
            self.nb_dauer -= self.tiefer_nb_dauer
            self.tiefer_zu_nacht = luft_round(self.T_NB_LW * self.volumen) + self.raum_druckstufe
            ...
            if abweichung < 0:
                ...
                self.tiefer_ab_nacht_ohne24h = self.abluft_labor_24h - self.ABL_24h
            else:
    ...
    def Druckstufe_Berechnen_neu(self):
        self.IGF_Druckstufe = self.raum_druckstufe
        n = abs(int(self.IGF_Druckstufe/10)) if abs(int(self.IGF_Druckstufe/10)) < 6 else 5
        if self.IGF_Druckstufe > 0:self.IGF_Legende = n*'+'
    ...
    def zuluft_min_berechnen(self):
        ...
        if zuluft != 0:
            zuluft = self.__lab_abl_24h_druckstufe_ueber_pruefen(zuluft)        
        angezuluft = self.__lab_abl_24h_druckstufe_ueber_pruefen(angezuluft)
 ...
 #       logger.info("Nachtbetrieb_Dauer = {}".format(nb_dauer))
    ...
    def zuluft_nacht_berechnen(self):
        ...
        if self.nachtbetrieb:
            ...
            zu_nacht = self.__lab_abl_24h_druckstufe_ueber_pruefen(zu_nacht)
...
#        logger.info("TieferNachtbetrieb_Dauer = {}".format(tiefer_nb_dauer))
    ...
    def tiefer_zuluft_nacht_berechnen(self):
        ...
        if self.nachtbetrieb:
            ...
            tiefer_zu_nacht = self.__lab_abl_24h_druckstufe_ueber_pruefen(tiefer_zu_nacht)
 ...
 #       logger.info("Tief_ZUL_Nacht = {}".format(tiefer_zu_nacht))
    ...
    def werte_schreiben(self):
        ...
        wert_schreiben("IGF_RLT_Luftmenge_RAB", self.abluft_ohne_24h)
        wert_schreiben("IGF_RLT_Luftmenge_RZU", self.zuluft_min)
        wert_schreiben("IGF_RLT_Luftmenge_24h", self.ABL_24h)
        wert_schreiben("IGF_RLT_Luftmenge_LAB", self.ABL_Labor)
...
with forms.ProgressBar(title="{value}/{max_value} Luftmengenberechnung",
    ...
    for n, space_id in enumerate(spaces):
        ...
        # mepraum.Tagesbetrieb_Berechnen()
        # mepraum.Nachtbetrieb_Berechnen()
        if mepraum.flaeche != 0:
