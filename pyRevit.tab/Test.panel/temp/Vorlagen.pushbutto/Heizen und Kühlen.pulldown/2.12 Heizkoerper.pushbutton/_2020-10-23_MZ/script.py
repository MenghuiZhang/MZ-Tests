...
__doc__ = """Schacht von Lüftung Übernehmen, HK_Heizleistung von MEP-Raum und Heizleistung bzw. Wassermenge von Schacht berechnen"""
...
with forms.ProgressBar(title='{value}/{max_value} Raumliste',
    ...
    for Space in spaces_collector:
        ...
        umluft_vl = get_value(Space.LookupParameter("IGF_H_Schacht_HK-VL"))
        umluft_rl = get_value(Space.LookupParameter("IGF_H_Schacht_HK-RL"))
...
if forms.alert('Schächte von Lüftung übernehmen?', ok=False, yes=True, no=True):
    ...
    with forms.ProgressBar(title='{value}/{max_value} Schächte übernehmen',
        ...
        for Spaces in spaces_collector:
            ...
            HK_VL_Schacht = Spaces.LookupParameter("IGF_H_Schacht_HK-VL")
            HK_RL_Schacht = Spaces.LookupParameter("IGF_H_Schacht_HK-RL")
            if Zuluft_Schacht and Zuluft_Schacht != 'None':
                HK_VL_Schacht.Set(str(Zuluft_Schacht))
            if Abluft_Schacht and Abluft_Schacht != 'None':
                HK_RL_Schacht.Set(str(Abluft_Schacht))
...
class MEPRaum:
    ...
    def __init__(self, element_id):
        ...
        attr = [
            ...
            ['Schacht_VL', 'IGF_H_Schacht_HK-VL'],
            ['Schacht_RL', 'IGF_H_Schacht_HK-RL'],
            ['Wassermenge', 'IGF_H_HK_Wassermenge'],
            ['Temp_VL', 'IGF_H_HK-VL_Temp'],
            ['Temp_RL', 'IGF_H_HK-RL_Temp'],
        ]
...
# HK_Menge_Liste erstellen
HK_M_VL_Liste = []
HK_M_RL_Liste = []

# HK_Leistung_Liste erstellen
HK_L_VL_Liste = []
HK_L_RL_Liste = []
...
for i in range(len(Schacht)):
    HK_M_VL_Liste.append(0)
    HK_M_RL_Liste.append(0)
    HK_L_VL_Liste.append(0)
    HK_L_RL_Liste.append(0)
        ...
        for SPACE in spaces_collector:
            ...
            for Raum_Date in table_data:
                if Raum_Date[0] == Raum_Nr:
                    SPACE.LookupParameter('IGF_H_HK_Leistung').SetValueString(str(Raum_Date[7]))
        t1.Commit()


# HK_Leistung berechnen
with forms.ProgressBar(title='{value}/{max_value} Heizleistung von Schacht Berechnen',
    ...
    for space in spaces_collector:
        ...
        HK_V_S = get_value(space.LookupParameter('IGF_H_Schacht_HK-VL'))
        HK_R_S = get_value(space.LookupParameter('IGF_H_Schacht_HK-RL'))
        Wassermenge = get_value(space.LookupParameter('IGF_H_HK_Wassermenge'))
        Leistung = get_value(space.LookupParameter('IGF_H_HK_Leistung'))

        for ii in range(len(Schacht)):
            if HK_V_S == Schacht[ii]:
                HK_M_VL_Liste[ii] = Wassermenge + HK_M_VL_Liste[ii]
                HK_L_VL_Liste[ii] = Leistung + HK_L_VL_Liste[ii]
            if HK_R_S == Schacht[ii]:
                HK_M_RL_Liste[ii] = Wassermenge + HK_M_RL_Liste[ii]
                HK_L_RL_Liste[ii] = Leistung + HK_L_RL_Liste[ii]

table_data_Schacht = []
for Nr in range(len(Schacht)):
    table_data_Schacht.append([Schacht[Nr], HK_M_VL_Liste[Nr], HK_M_RL_Liste[Nr],HK_L_VL_Liste[Nr], HK_L_RL_Liste[Nr],])
...
output.print_table(
    ...
    title="Schächte",
    columns=[ 'Schacht','HK-VL_Menge','HK-RL_Menge','HK-VL_Leistung','HK-RL_Leistung',  ]
)
            ...
            if Ins_Schacht:
                for num in range(len(Schacht)):
                    if Ins_Schacht_Name == Schacht[num]:
                        Schacht_V = SPACE.LookupParameter('IGF_H_Schacht_HK-VL_Menge')
                        Schacht_R = SPACE.LookupParameter('IGF_H_Schacht_HK-RL_Menge')
                        SchachtV = SPACE.LookupParameter('IGF_H_Schacht_HK-VL_Leistung')
                        SchachtR = SPACE.LookupParameter('IGF_H_Schacht_HK-RL_Leistung')
                        Schacht_V.SetValueString(str(HK_M_VL_Liste[num]))
                        Schacht_R.SetValueString(str(HK_M_RL_Liste[num]))
                        SchachtV.SetValueString(str(HK_L_VL_Liste[num]))
                        SchachtR.SetValueString(str(HK_L_RL_Liste[num]))
