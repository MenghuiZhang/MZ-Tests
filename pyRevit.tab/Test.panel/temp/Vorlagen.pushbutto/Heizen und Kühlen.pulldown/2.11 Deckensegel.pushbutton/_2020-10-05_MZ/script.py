...
__doc__ = """Schacht von Lüftung Übernehmen und Heizung-Leistung der Deckensegel in MEP-Raum bzw. Wassermenge in Schacht berechnen"""
...
class MEPRaum:
    ...
    def __init__(self, element_id):
        ...
        attr = [
            ...
            ['Leistung_VL', 'IGF_H_Schacht_DeS-VL_Leitung'],
            ['Leistung_RL', 'IGF_H_Schacht_DeS-RL_Leitung'],
    ...
    def LeistungBerechnen(self):
        ...
        if self.Leistung_VL and self.Leistung_RL and self.Temp_VL and self.Temp_RL:
            leistung = 4200 * self.Leistung_VL * (self.Temp_VL- self.Temp_RL)
...
with forms.ProgressBar(title='{value}/{max_value} Wärmeabgabe Berechnen',
    ...
    for space in spaces_collector:
        ...
        V_Leistung = get_value(space.LookupParameter('IGF_H_Schacht_DeS-VL_Leitung'))
        DeS_R_S = get_value(space.LookupParameter('IGF_H_Schacht_DeS-RL'))
        R_Leistung = get_value(space.LookupParameter('IGF_H_Schacht_DeS-RL_Leitung'))

        for ii in range(len(Schacht)):
            if DeS_V_S == Schacht[ii]:
                DeS_VL_Liste[ii] = V_Leistung + DeS_VL_Liste[ii]
            if DeS_R_S == Schacht[ii]:
                DeS_RL_Liste[ii] = R_Leistung + DeS_RL_Liste[ii]
...
output.print_table(
    ...
    title="Mep Räume",
    columns=['Nummer', 'Name',  'Schacht_VL', 'Schacht_RL', 'Leistung_VL',
             'Leistung_RL', 'Temp_VL', 'Temp_RL', 'DeS_Heizung', 'Heizlast']
)
        ...
        for SPACE in spaces_collector:
            ...
            for Raum_Date in table_data:
                if Raum_Date[0] == Raum_Nr:
                    SPACE.LookupParameter('IGF_H_DeS_Leistung').SetValueString(str(Raum_Date[8]))
            if Ins_Schacht:
