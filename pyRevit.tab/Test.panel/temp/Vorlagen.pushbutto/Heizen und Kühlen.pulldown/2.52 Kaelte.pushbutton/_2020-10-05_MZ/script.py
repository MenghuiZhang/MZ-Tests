...
__title__ = "Kälte"
__doc__ = """Schacht von Lüftung Übernehmen und Kälte-Leistung von MEP-Raum bzw. Wassermenge von Schacht berechnen"""
...
# Schachtsname von Zu- und Abluft jeKA MEP Raum lesen
tabel_Date_MEP = []
schacht = []
with forms.ProgressBar(title='{value}/{max_value} Räume',
...
for i in range(len(Schacht)):
    KA_VL_Liste.append(0)
    KA_RL_Liste.append(0)
    ...
    for space in spaces_collector:
        ...
        for ii in range(len(Schacht)):
            if KA_V_S == Schacht[ii]:
                KA_VL_Liste[ii] = Wassermenge + KA_VL_Liste[ii]
            if KA_R_S == Schacht[ii]:
                KA_RL_Liste[ii] = Wassermenge + KA_RL_Liste[ii]

table_data_Schacht = []
for Nr in range(len(Schacht)):
    table_data_Schacht.append([Schacht[Nr], KA_VL_Liste[Nr], KA_RL_Liste[Nr],])

output.print_table(
    table_data=table_data,
    title="Mep Räume",
    columns=['Nummer', 'Name',  'Schacht_VL', 'Schacht_RL', 'Wassermenge',
             'Temp_VL', 'Temp_RL', 'Kühlleistung', ]
)

output.print_table(
    table_data=table_data_Schacht,
    title="Schächte",
    columns=[ 'Schacht','KA-VL_Win_Menge','KA-RL_Win_Menge',  ]
)
...
#                        Schacht_V.SetValueString(str(KA_VL_Liste[num]))
#                        Schacht_R.SetValueString(str(KA_RL_Liste[num]))
                        SchachtV.SetValueString(str(KA_VL_Liste[num]))
                        SchachtR.SetValueString(str(KA_RL_Liste[num]))
