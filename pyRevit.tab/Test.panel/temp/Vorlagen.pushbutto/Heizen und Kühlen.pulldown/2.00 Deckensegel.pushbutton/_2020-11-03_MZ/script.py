    ...
    def __init__(self, element_id):
        ...
        if self.V_H:
            ...
            if self.nummer and self.name:
                self.Raum_Liste.append([self.nummer, self.name, self.Volumen,
                                        self.V_H, self.R_H, self.HL, self.V_K,
                                        self.R_K, self.KL])
...
table_Data = [[table_data[0][0], table_data[0][1], 0, table_data[0][3],
               table_data[0][4], 0, table_data[0][6],table_data[0][7], 0]]

for i in range(1,len(table_data)):
    j = i - 1
    if str(table_data[i][0]) != str(table_data[j][0]):
        table_Data.append([table_data[i][0], table_data[i][1], 0, table_data[i][3],
                          table_data[i][4], 0, table_data[i][6], table_data[i][7], 0])
...
for data in table_data:
    for room in table_Data:
        if data[0] == room[0]:
            ...
            room[5] = room[5] + data[5]
            room[8] = room[8] + data[8]

output.print_table(
    table_data=table_Data,
    title="Raum",
    columns=['Nummer','Name','Volumen','Vor_Heizen','Rück_Heizen','Heizleistung',
             'Vor_Kühlen','Rück_Kühlen', 'Kühlleistung']
)
...
if forms.alert('Temperatur und Volemenstrom übernehmen?', ok=False, yes=True, no=True):
    ...
    with forms.ProgressBar(title='{value}/{max_value} Temperatur, Volumenstrom, Heiz- und Kühlleistung schreiben',
                           cancellable=True, step=10) as pb1:
        n_1 = 0

        t = Transaction(doc, "Daten übernehmen")
        t.Start()
        ...
        for Spaces in spaces_collector:
            ...
            for Date in table_Data:
                if Nummer == Date[0]:
                    ...
                    Spaces.LookupParameter("IGF_K_DeS-VL_Som_Temp").SetValueString(str(Date[6]))
                    Spaces.LookupParameter("IGF_K_DeS-RL_Som_Temp").SetValueString(str(Date[7]))
                    ...
                    Spaces.LookupParameter('IGF_H_DeS_Leistung').SetValueString(str(Date[5]))
                    Spaces.LookupParameter('IGF_K_DeS_Leistung').SetValueString(str(Date[8]))
