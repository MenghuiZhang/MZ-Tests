...
class MEPRaum:
    ...
    def __init__(self, element_id):
        ...
        attr = [
            ...
            ['AB', 'IGF_RLT_AbluftminRaumOhne24h'],
        ...
        if self.ZU > 0 and self.Zu_Anl_Nr is None and self.ZU_S:
            logger.error("Zuluft_Anlage_Nummer in Raum {} nicht gefunden".format(self.nummer))
            script.exit()

        if self.AB > 0 and self.Ab_Anl_Nr is None and self.AB_S:
            logger.error("Abluft_Anlage_Nummer in Raum {} nicht gefunden".format(self.nummer))
            script.exit()

        if self.AB24H > 0 and self.Ab_Anl_Nr_24h is None and self.AB24H_S:
            logger.error("Abluft24h_Anlage_Nummer in Raum {} nicht gefunden".format(self.nummer))
            script.exit()
...
mepraum_liste = []
with forms.ProgressBar(title='{value}/{max_value} MEP Räume Liste',
...
with forms.ProgressBar(title='{value}/{max_value} Berechnen',
    ...
    for Space in spaces_collector:
        ...
        Zul = get_value(Space.LookupParameter('IGF_RLT_AnlagenRaumZuluft'))
        Abl = get_value(Space.LookupParameter('IGF_RLT_AnlagenRaumAbluft'))
        Abl_24h = get_value(Space.LookupParameter('IGF_RLT_AnlagenRaum24hAbluft'))
...
with forms.ProgressBar(title='{value}/{max_value} Parameterwerte erstellen',
            ...
            for num in range(len(SchachtListe)):
                if Ins_Schacht_Name == SchachtListe[num]:
                    ...
                    for aaa in range(len(EbenenListe)):
                        ...
                        if any(ebe_zu):
                            if luft_zu == '':
                                luft_zu = luft_zu + '[m3/h] - ' + str(ebene) +': '
                                for bbb in range(len(ZuAnlNrListe)):
                                    if ebe_zu[bbb] > 0:
                                        luft_zu = luft_zu + 'Anl ' + str(ZuAnlNrListe[bbb]) + '=' + str(int(round(ebe_zu[bbb]))) + ', '
                        ...
                        if any(ebe_ab):
                            if luft_ab == '':
                                luft_ab = luft_ab + '[m3/h] - ' +str(ebene) +': '

                                for ccc in range(len(AbAnlNrListe)):
                                    if ebe_ab[ccc] > 0:
                                        luft_ab = luft_ab + 'Anl ' + str(AbAnlNrListe[ccc]) + '=' + str(int(round(ebe_ab[ccc]))) + ', '
                        ...
                        if any(ebe_ab24h):
                            if luft_ab24h == '':
                                luft_ab24h = luft_ab24h + '[m3/h] - ' + str(ebene) +': '

                                for ddd in range(len(Ab24hAnlNrListe)):
                                    if ebe_ab24h[ddd] > 0:
                                        luft_ab24h = luft_ab24h + 'Anl ' + str(Ab24hAnlNrListe[ddd]) + '=' + str(int(round(ebe_ab24h[ddd]))) + ', '
...
def Table_row():
    """ Gibt eine Datenreihe für Schacht_Liste aus. Für die tabellarische Übersicht."""
    return [
        SchachtListe[Nr],
        Zu_Verteilen[Nr],
        Ab_Verteilen[Nr],
...
table_data_Schacht = []
Nr = 0
for Nr in range(len(SchachtListe)):
    table_data_Schacht.append([SchachtListe[Nr],Zu_Verteilen[Nr][:-2],Ab_Verteilen[Nr][:-2],Ab24h_Verteilen[Nr][:-2]])
    ...
    with forms.ProgressBar(title='{value}/{max_value} Werte schreiben',
        ...
        for SPACE in spaces_collector:
            ...
            if pb3.cancelled:
                script.exit()
            n_2 += 1
            pb3.update_progress(n_2, len(spaces))
            ...
            if INS_Schacht:
                for NUM in range(len(SchachtListe)):
                    if INS_Schacht_Name == SchachtListe[NUM]:
                        ...
                        print(Zu_Verteilen[NUM][:-2])
                        Zul_Verteilung.Set(Zu_Verteilen[NUM][:-2])

                        Abl_Verteilung = SPACE.LookupParameter('IGF_RLT_VerteilungAbluft')
                        Abl_Verteilung.Set(Ab_Verteilen[NUM][:-2])

                        Ab24h_Verteilung = SPACE.LookupParameter('IGF_RLT_Verteilung24hAbluft')
                        Ab24h_Verteilung.Set(Ab24h_Verteilen[NUM][:-2])
