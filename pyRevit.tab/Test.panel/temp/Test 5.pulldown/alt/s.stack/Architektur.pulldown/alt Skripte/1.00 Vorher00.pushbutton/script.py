...
def MEP_Raum(Familie,Familie_Id):
    ...
    with forms.ProgressBar(title='{value}/{max_value} Raumzustand übernehmen',
        ...
        for raum in Familie:
            ...
            # if raum.LookupParameter('IGF_A_Raumnummer_Vorher00'):
            #     raum.LookupParameter('IGF_A_Raumnummer_Vorher00').Set(Nummer)
            # if raum.LookupParameter('IGF_A_Raumname_Vorher00'):
            #     raum.LookupParameter('IGF_A_Raumname_Vorher00').Set(Name)
            # if raum.LookupParameter('IGF_A_Fläche_Vorher00'):
            #     raum.LookupParameter('IGF_A_Fläche_Vorher00').SetValueString(str(Flaeche))
            # if raum.LookupParameter('IGF_A_Volumen_Vorher00'):
            #     raum.LookupParameter('IGF_A_Volumen_Vorher00').SetValueString(str(Volumen))
            # if raum.LookupParameter('IGF_A_Umfang_Vorher00'):
            #     raum.LookupParameter('IGF_A_Umfang_Vorher00').SetValueString(str(Umfang))
            # if raum.LookupParameter('IGF_A_LichteHöhe_Vorher00'):
            #     raum.LookupParameter('IGF_A_LichteHöhe_Vorher00').SetValueString(str(Hoehe))
            if raum.LookupParameter('IGF_A_Datum_Vorher00'):
                raum.LookupParameter('IGF_A_Datum_Vorher00').Set(Zustand)
