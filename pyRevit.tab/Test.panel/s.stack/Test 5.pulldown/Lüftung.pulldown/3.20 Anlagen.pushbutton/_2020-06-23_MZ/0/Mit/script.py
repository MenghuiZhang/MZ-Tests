...
if len(Fehlende_Info) > 0:
    logger.warning('keine Informationen zur Geräteanzahl/AnlagenProzentualAnzahl/Gerätenummer in den folgenden Systemen')
    logger.warning('{}'.format(Fehlende_Info))
...
if len(Fehlende_Info) > 0:
    if forms.alert('Möchten Sie Geräteanzahl=1 und AnlagenProAnzahl=1 für die obengenannten Systemen berechnen? Diese werte werden nicht in Modell schreiben.', ok=False, yes=True, no=True):
