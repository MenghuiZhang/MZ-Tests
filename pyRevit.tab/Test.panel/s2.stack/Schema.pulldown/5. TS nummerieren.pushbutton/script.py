...
el_system = el0.MEPSystem

elems = el_system.PipingNetwork
...
for el in elems:
    if el.Category.Id.ToString() == '-2001140':
        try:
            nr = el.LookupParameter(config.param).AsString()
            #nr = el.LookupParameter('IGF_X_Bauteil_ID_Text').AsString()
            hls_liste[nr] = el
        except:pass
    elif el.Category.Id.ToString() == '-2008044':
        rohr.append(el.Id.ToString())
    elif el.Category.Id.ToString() == '-2008049':
        formteil.append(el.Id.ToString())
    elif el.Category.Id.ToString() == '-2008055':
        zubehoer.append(el.Id.ToString())
            ...
            if conns:
                ...
                for conn in conns:
                    ...
                    if elem.Id.ToString() != self.elem.Id.ToString():
                        ...
                        for ref in refs:
                            owner = ref.Owner
                            if owner.Id.ToString() in rohr or owner.Id.ToString() in formteil or owner.Id.ToString() in zubehoer:
