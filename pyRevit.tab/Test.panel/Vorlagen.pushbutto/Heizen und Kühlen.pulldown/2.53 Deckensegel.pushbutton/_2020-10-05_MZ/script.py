...
__doc__ = """Schacht von Lüftung Übernehmen und Kühlleistung der Deckensegel von MEP-Raum bzw. Wassermenge von Schacht berechnen"""
...
with forms.ProgressBar(title='{value}/{max_value} Räume',
    ...
    for Space in spaces_collector:
        ...
        umluft_vl = get_value(Space.LookupParameter("IGF_K_Schacht_DeS-VL"))
        umluft_rl = get_value(Space.LookupParameter("IGF_K_Schacht_DeS-RL"))
...
if forms.alert('Schächte von Lüftung übernehmen?', ok=False, yes=True, no=True):
    ...
    with forms.ProgressBar(title='{value}/{max_value} Schächte übernehmen',
        ...
        for Spaces in spaces_collector:
            ...
            DeS_VL_Schacht = Spaces.LookupParameter("IGF_K_Schacht_DeS-VL")
            DeS_RL_Schacht = Spaces.LookupParameter("IGF_K_Schacht_DeS-RL")
...
class MEPRaum:
    ...
    def __init__(self, element_id):
        ...
        attr = [
            ...
            ['Schacht_VL', 'IGF_K_Schacht_DeS-VL'],
            ['Schacht_RL', 'IGF_K_Schacht_DeS-RL'],
            ['Wassermenge', 'IGF_K_DeS_Sommer'],
            ['Temp_VL', 'IGF_K_DeS-VL_Som_Temp'],
            ['Temp_RL', 'IGF_K_DeS-RL_Som_Temp'],
        ]
...
for i in range(len(Schacht)):
    DeS_VL_Liste.append(0)
    DeS_RL_Liste.append(0)
...
with forms.ProgressBar(title='{value}/{max_value} Wärmeabgabe Berechnen',
    ...
    for space in spaces_collector:
        ...
        DeS_V_S = get_value(space.LookupParameter('IGF_K_Schacht_DeS-VL'))
        DeS_R_S = get_value(space.LookupParameter('IGF_K_Schacht_DeS-RL'))
        Wassermenge = get_value(space.LookupParameter('IGF_K_DeS_Sommer'))

        for ii in range(len(Schacht)):
            if DeS_V_S == Schacht[ii]:
                DeS_VL_Liste[ii] = Wassermenge + DeS_VL_Liste[ii]
            if DeS_R_S == Schacht[ii]:
                DeS_RL_Liste[ii] = Wassermenge + DeS_RL_Liste[ii]

table_data_Schacht = []
for Nr in range(len(Schacht)):
    table_data_Schacht.append([Schacht[Nr], DeS_VL_Liste[Nr], DeS_RL_Liste[Nr],])
...
output.print_table(
    ...
    title="Schächte",
    columns=[ 'Schacht','DeS-VL_Win_Menge','DeS-RL_Win_Menge',  ]
)
        ...
        for SPACE in spaces_collector:
            ...
            for Raum_Date in table_data:
                if Raum_Date[0] == Raum_Nr:
                    SPACE.LookupParameter('IGF_K_DeS_Leistung').SetValueString(str(Raum_Date[7]))
            if Ins_Schacht:
                for num in range(len(Schacht)):
                    if Ins_Schacht_Name == Schacht[num]:
                        Schacht_V = SPACE.LookupParameter('IGF_K_Schacht_DeS-VL_Som_Menge')
                        Schacht_R = SPACE.LookupParameter('IGF_K_Schacht_DeS-RL_Som_Menge')
                        SchachtV = SPACE.LookupParameter('IGF_K_Schacht_DeS-VL_Leistung')
                        SchachtR = SPACE.LookupParameter('IGF_K_Schacht_DeS-RL_Leistung')
                        Schacht_V.SetValueString(str(DeS_VL_Liste[num]))
                        Schacht_R.SetValueString(str(DeS_RL_Liste[num]))
                        SchachtV.SetValueString(str(DeS_VL_Liste[num]))
                        SchachtR.SetValueString(str(DeS_RL_Liste[num]))
